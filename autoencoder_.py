...
class FragmentedVectorQuantizer(tf.keras.layers.Layer):
    def __init__(self, num_embeddings, embedding_dim, codebook_resolution=4, beta=0.25, **kwargs):
        super().__init__(**kwargs)
        self.embedding_dim = embedding_dim
        self.num_embeddings = num_embeddings

        self.codebook_resolution = codebook_resolution
        assert embedding_dim % codebook_resolution == 0
        self.code_size = embedding_dim // codebook_resolution
        ...
        self.embeddings = tf.Variable(
            initial_value=w_init(
                shape=(self.code_size, self.num_embeddings), dtype='float32'
            ),
            trainable=True,
            name='embeddings_vqvae',
        )
    ...
    def call(self, x, return_indices=False):
        ...
        flattened = tf.reshape(x, [-1, self.embedding_dim])
        quantized = []
        for i in range(self.codebook_resolution):
            flattened_fragment = flattened[:, i*self.code_size:(i+1)*self.code_size]
            encoding_indices = self.get_code_indices(flattened_fragment)
            encodings = tf.one_hot(encoding_indices, self.num_embeddings)
            quantized_fragment = tf.matmul(encodings, self.embeddings, transpose_b=True)
            quantized.append(quantized_fragment)
        quantized = tf.concat(quantized, axis=1)
...
class VQVariationalAutoencoder(tf.keras.Model):
    def __init__(self, input_size=700, num_embeddings=64, encoder_layer_sizes=[1], decoder_layer_sizes=[700], name='vqvae'):
        ...
        latent_dim = encoder_layer_sizes[-1]
        autoencoder = Autoencoder(input_size=700, encoder_layer_sizes=encoder_layer_sizes, decoder_layer_sizes=decoder_layer_sizes)
        # self.vq_layer = FragmentedVectorQuantizer(num_embeddings, latent_dim, codebook_resolution=1, name='vector_quantizer')
